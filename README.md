...
c3_checkpointsContract is the contract where the verification of the prover's VDF occurs. The prover has published a final result in R5 of every box under c3_checkpointsContract with 1 proof token in each box along with a series of checkpoint values which show the prover's progression from the seed to the result. The proof is assumed true if after 200 blocks (this value may be changed and made variable in future) the prover is able to combine all 360 proof tokens together in one result box. However, anyone in the public is able to dispute the prover's claim by attempting to burn any of the proof tokens. If the prover's output is incorrect, or if any checkpoint are incorrect, then it will be possible for the public to burn a proof token (and claim the slash reward as an incentive to verify VDF claims) and thus invalidate the VDF proof. 

Thus c3_checkpointsContract has two possible outcomes:
1. Undisputed VDF: The VDF prover can (after 200 blocks) combine all 360 proof tokens under a result address. dApps can query this result address and easily validate that the VDF is correct by checking the encoded box in R7 has has the same id as the proof token and that the encoded box has an address of c1_proofTokenMinter and that all 360 proof tokens are present. The VDF prover can claim all the ERG held under c2_proofDistribution, less any box MinErgs and mining fees. 
2. Disputed VDF: The VDF prover will not be able to combine all 360 proof tokens and thus a dApp should reject the VDF result. The entity that successfuly disputed the VDF can claim the slashed ERG held under c3_checkpointsContract and they will burn the proof token as per the conditions of c3_checkpointsContract. The dApp can make another request for a VDF output by simply funding another VDF mint box. 
