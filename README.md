...
2. Disputed VDF: The VDF prover will not be able to combine all 360 proof tokens and thus a dApp should reject the VDF result. The entity that successfuly disputed the VDF can claim the slashed ERG held under c2_proofDistribution and they will burn the proof token as per the conditions of c3_checkpointsContract. The dApp can make another request for a VDF output by simply funding another VDF mint box. 
